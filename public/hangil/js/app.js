...
const ROUTES = [
  ...
  [/^#\/vocab\/([\w-]+)\/learn$/,  (m) => V.vocabLearn(root, m[1])],
  ...
  [/^#\/trade\/([\w-]+)$/,       (m) => V.trade(root, m[1])],
  [/^#\/trade\/([\w-]+)\/drill$/, (m) => V.tradeDrill(root, m[1])],
  [/^#\/trade\/([\w-]+)\/words$/, (m) => V.tradeWords(root, m[1])],
  [/^#\/guide$/,                 () => V.guide(root)],
  [/^#\/weak$/,                  () => V.weak(root)],
  [/^#\/weak\/([\w-]+)$/,        (m) => V.weakDrill(root, m[1])],
