...
import { h, clear, mmss, today } from './util.js';
...
export function runMock(root, m, { head, nav }) {
  ...
  const items = [
    ...
    ...m.reading.map((it, i) => ({ ...it, ...shuffled(it), section: 'reading', n: m.listening.length + i + 1 })),
  ];
  ...
  function submit(ranOut) {
    ...
    let lis = 0, read = 0;
    items.forEach((it, n) => {
      if (answers[n] === it.answer) { if (it.section === 'listening') lis += 1; else read += 1; }
    });
    ...
    items.forEach((it, n) => {
      ...
      const card = h('div', { class: 'card' },
        h('p', { class: 'card__meta' }, `${it.n} · ${it.section === 'listening' ? '듣기' : '읽기'} · ${answers[n] === null ? 'blank' : ok ? 'correct' : 'wrong'}`),
      );
